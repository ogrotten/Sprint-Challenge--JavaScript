...
console.log(`${dino1.name} weighed ${dino1.weight}`);

// What was the diet of a velociraptor?
console.log(`${dino3.name} diet was ${dino3.diet}`);

// How long was a stegosaurus?
console.log(`${dino2.name} length was ${dino2.length}`);

// What time period did tyrannosaurus live in?
console.log(`${dino1.name} time period was ${dino1.period}`);

// Create a new roar method for the tyrannosaurus.  When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
dino1.roar = function(noise) {
	return `${noise}`;
};
console.log(dino1.roar("RAWERSRARARWERSARARARRRR I'm big n mean mawp."));
...
const unisWithUni = [];
graduates.forEach(e => {
	e.university.includes("Uni")
		? unisWithUni.push({ university: e.university })
		: "";
});
...
zooAnimals.forEach(e => {
	displayNames.push(
		`Name: ${e.animal_name}; Scientific: ${e.scientific_name}.`
	);
});
...
The zoos need a list of all their animal's names (animal_name only) converted to lower case. Using map, create a new array of strings named lowCaseAnimalNames, each string following this pattern:
"jackal, asiatic". Log the resut.

*/

let lowCaseAnimalNames = [];
lowCaseAnimalNames = zooAnimals.map(e => `${e.animal_name.toLocaleLowerCase()}`);
...
populationTotal = zooAnimals.reduce((acc, e) => acc + e.population, 0)
